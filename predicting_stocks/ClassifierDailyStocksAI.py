...
def fit_data(ticker, start_day, end_day, data=None):
    """
    ...
    """
    train_data = get_data(ticker, start_day, end_day, data_that_given=data)
...
def return_test_data(test_start, test_end, prediction_days, prediction_day, ticker, scalar):
    ...
    test_data = pd.DataFrame(get_data(ticker, start_day=test_data_time, end_day=test_end, data_that_given=read_csv_file())).values
    print(test_data)
    actual_data = []
...
def build_model_for_multiple_prediction(ticker, prediction_day=None,
    ...
    scaled_data, scalar = fit_data(ticker, start_day=start_day, end_day=end_day, data=data_that_fits)
    x_train, y_train = prepare_data(scaled_data, prediction_days, prediction_day)
...
def main():
    ...
    model = build_model_for_multiple_prediction(ticker, prediction_days=1000, prediction_day=1, data_that_fits=read_csv_file())
    predict_stock_price_at_specific_day(ticker, prediction_days=1000, prediction_day=1, model=model)
    p, r = test_model(ticker, prediction_days=1000, prediction_day=1, model=model)
