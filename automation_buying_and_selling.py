...
STOCKS_PREDICTED = {'NIO':[42], 'XPEV':[43], 'LI':[29]}
RISK = 1.015
PROFIT = 1.03
...
def stocks_worth_trading(stocks_prediction, profit=PROFIT, ):
    ...
    return dict((ticker, stocks_prediction[ticker])
                ...
                (ratio := max((prices := stocks_prediction[ticker])[0] / prices[1],
                              prices[1] / prices[0])) >= get_last_price(ticker) * profit
                or ratio <= get_last_price(ticker) * (2 - profit))
